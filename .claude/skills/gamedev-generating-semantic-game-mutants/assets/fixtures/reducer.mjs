// Reference (clean) reducer for the mutant fixtures.
...
export function reduce(prev, ev) {
  ...
  switch (ev.type) {
    ...
    case "spawn":
      // A spawn always produces a fresh identity, even into a slot used before.
    ...
    case "charge":
      // Pressing again during a hold does not restart it.
      if (s.charging) break;
      s.charging = true;
    ...
    case "release": {
      ...
      // One hold arms one shot, whether or not that shot lands.
      s.charging = false;
      s.chargeStart = -1;
