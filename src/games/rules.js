...
export const RULES = {
  ...
  paroles: {
    ...
    fields: (c) => [
      ['🚀 Lancer', '`/jeu-paroles` (menu : thème, difficulté, manches) ou directement avec les options `theme`, `difficulte`, `manches`.\nC\'est aussi un mode du blind test : `/jeu-blindtest mode:Suite des paroles`.'],
  ...
  histoire: {
    ...
    fields: (c) => [
      ['🚀 Lancer', '`/jeu-histoire` avec les options `univers` (Fantasy, Horreur, Braquage, Zombies, Espace, Animé, Rap game), `mode` et `longueur` (5, 8 ou 12 chapitres).'],
      ...
      ['🎙️ Mode 100 % vocal', `Allez tous dans <#${c.iaVocal}>, lancez \`/jeu-histoire mode:100 % vocal\` : l'IA raconte à voix haute et vous lui parlez directement. 25 minutes maximum, dites « on arrête » pour finir.`],
  ...
  fans: {
    ...
    fields: (c) => [
      ['🚀 Lancer', `\`/jeu-fans\` avec l'option \`theme\` : Rap FR, Stars du monde ou Tout mélangé. Tu peux jouer dans <#${c.mini}>, tout le monde voit ta série.`],
  ...
  annee: {
    ...
    fields: (c) => [
      ['🚀 Lancer', '`/jeu-annee` (menu : thème, difficulté, manches) ou avec les options. C\'est aussi un mode du blind test.'],
  ...
  rebus: {
    ...
    fields: (c) => [
      ['🚀 Lancer', `\`/jeu-rebus\` avec les options \`theme\` et \`manches\` (3 à 20). La partie se joue dans <#${c.mini}>.`],
  ...
  fantasy: {
    ...
    fields: (c) => [
      ['🚀 Ton équipe', '`/jeu-fantasy equipe` et choisis tes 5 rappeurs (le bot te propose les noms en tapant). Sans option : tu vois ton équipe et tes points.\nDeux joueurs ne peuvent pas avoir exactement la même équipe.'],
      ...
      ['🏆 Classement', `\`/jeu-fantasy classement\` à tout moment. Chaque lundi, les points et les rappeurs de la semaine sont publiés dans <#${c.mini}>. Le 1er du mois, le champion est sacré.`],
    ],
  },
  battle: {
    ...
    fields: (c) => [
      ['⚖️ Lancement', `Les juges lancent la battle avec \`/tribunal action:battle\` et choisissent deux sons validés de la semaine. Elle est publiée dans <#${c.annonces}>.`],
